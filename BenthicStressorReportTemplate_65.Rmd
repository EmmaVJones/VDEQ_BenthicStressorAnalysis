---
title: "Benthic Stressor Analysis Report Template- VCPMI (Ecoregion 65 minus Chowan)"
...
bugData <- params[['userDataRMD']][['bugData']] %>%
  dplyr::rename("VCPMI"="CPMI65-CHOWAN")
...
#bugData <- read_excel('C:/HardDriveBackup/R/ShinyApp_TMDLworkgroup/BenthicStressorTool/testData/7-PAR003.09/Family Metrics VCPMI65-Chowan_MRD_7-PAR003.09.xlsx') %>%
#  dplyr::rename("VCPMI"="CPMI65-CHOWAN")
...
![Table 4: Metrics used to calculate the Virginia Coastal Plain Macroinvertebrate Index (VCPMI) (VDEQ 2013).](Table1_VCPMI65metrics.JPG)
...
bugScores <- select(bugData, SampleSeason,RichnessFinal,EPTFinal,EPHEM,`PT-H`,HBIFinal,
                    PctIntol,`PctClng-HS`,PctScrap,`VCPMI`)
...
bugScores[1:nrow(bugScores),2:10]%>% 
  datatable(rownames =  as.character(bugScores$SampleSeason),
            ...
            "HBI","% Intolerant Taxa","% Clingers - Hydropsychidae + Simuliidae","% Scrapers","VCPMI"),
            options=list(dom= 't',
  ...
  formatRound(c('RichnessFinal','EPTFinal','EPHEM','PT-H','HBIFinal','PctIntol',
                    'PctClng-HS','PctScrap','VCPMI'), 2)
